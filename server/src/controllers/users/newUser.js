...
const newUser = async (req, res, next) => {
  try {
    const { email, username, ownername, password, role } = req.body;

    await validateSchema(newUserSchema, req.body);

    const regCode = uuid();

    await insertUserQuery(email, username, ownername, password, role, regCode);

    const activationURL = `http://localhost:8000/users/validate/${regCode}`;

    const emailSubject = 'Activación de usuario en SocialDoby';
    const emailBody = `
        <!DOCTYPE html>
        <html>
        <head>
